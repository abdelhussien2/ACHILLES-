...
async def simple_search(page, query):
    """Original March flow: direct URL search, no humanization."""
    t = time.time()
    log.info(f"  [SEARCH-SIMPLE] \"{query}\"")
    ...
    return await _parse_results(page, t)


async def _parse_results(page, t):
    """Wait for results, return parsed cards."""
    try:
...
async def simple_click_product(page, card, asin):
    """Original March click flow: just el.click(), no mouse movement."""
    for name, sel in [
